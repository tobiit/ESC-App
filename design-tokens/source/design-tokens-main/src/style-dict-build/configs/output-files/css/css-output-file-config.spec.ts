import { beforeEach, describe, expect, it, jest } from '@jest/globals';
import { File } from 'style-dictionary';
import { excludedTokensFilterName } from '../../../hooks/index.js';
import { cssOutputFileConfig } from './css-output-file-config.js';
import {
	createDifferentPathCssConfigScenario,
	createEmptyPathCssConfigScenario,
	createNullInputScenario,
	createSpacesInPathCssConfigScenario,
	createSpecialCharsInPathCssConfigScenario,
	createStandardCssConfigScenario,
	createUndefinedInputScenario,
	mockMockedCssVariablesFormat,
	mockMockedExcludedTokensFilterName,
} from './css-output-file-config.mock.spec.js';

/**
 * DISCLAIMER: This file has been generated using AI assistance.
 * The generation process was guided by the prompt specified in `.github/prompts/unit-testing.prompt.md`.
 */

// Mock the hooks dependency
jest.mock('../../../hooks/index.js', () => {
	return {
		excludedTokensFilterName: 'mocked-filter-name',
	};
});

// Mock style-dictionary/enums to avoid ES module issues
jest.mock('style-dictionary/enums', () => {
	return {
		formats: {
			cssVariables: 'css/variables',
		},
	};
});

// Cast mocked function with proper typing
const mockExcludedTokensFilterName = excludedTokensFilterName as string;

describe('cssOutputFileConfig', () => {
	beforeEach(() => {
		jest.clearAllMocks();
	});

	describe('successful config generation', () => {
		it('should generate CSS output file config with standard file path', () => {
			const scenario = createStandardCssConfigScenario();

			const result = cssOutputFileConfig(scenario.input);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0]).toEqual({
				destination: `${scenario.input}.css`,
				format: mockMockedCssVariablesFormat,
				filter: mockExcludedTokensFilterName,
				options: {
					selector: [':root, :host'],
				},
			});
		});

		it('should generate CSS output file config with different file path', () => {
			const scenario = createDifferentPathCssConfigScenario();

			const result = cssOutputFileConfig(scenario.input);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0]).toEqual({
				destination: `${scenario.input}.css`,
				format: mockMockedCssVariablesFormat,
				filter: mockExcludedTokensFilterName,
				options: {
					selector: [':root, :host'],
				},
			});
		});

		it('should handle empty string input gracefully', () => {
			const scenario = createEmptyPathCssConfigScenario();

			const result = cssOutputFileConfig(scenario.input);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0]).toEqual({
				destination: '.css',
				format: mockMockedCssVariablesFormat,
				filter: mockExcludedTokensFilterName,
				options: {
					selector: [':root, :host'],
				},
			});
		});

		it('should handle file paths with spaces', () => {
			const scenario = createSpacesInPathCssConfigScenario();

			const result = cssOutputFileConfig(scenario.input);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0]).toEqual({
				destination: `${scenario.input}.css`,
				format: mockMockedCssVariablesFormat,
				filter: mockExcludedTokensFilterName,
				options: {
					selector: [':root, :host'],
				},
			});
		});

		it('should handle file paths with special characters', () => {
			const scenario = createSpecialCharsInPathCssConfigScenario();

			const result = cssOutputFileConfig(scenario.input);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0]).toEqual({
				destination: `${scenario.input}.css`,
				format: mockMockedCssVariablesFormat,
				filter: mockExcludedTokensFilterName,
				options: {
					selector: [':root, :host'],
				},
			});
		});
	});

	describe('return value structure validation', () => {
		it('should return array of File objects with correct properties', () => {
			const testPath = '/test/path';
			const result = cssOutputFileConfig(testPath);

			expect(Array.isArray(result)).toBe(true);
			expect(result).toHaveLength(1);

			const file = result[0] as File;
			expect(file).toHaveProperty('destination');
			expect(file).toHaveProperty('format');
			expect(file).toHaveProperty('filter');

			expect(typeof file.destination).toBe('string');
			expect(typeof file.format).toBe('string');
			expect(typeof file.filter).toBe('string');
		});

		it('should use correct CSS format from style-dictionary', () => {
			const testPath = '/test/path';
			const result = cssOutputFileConfig(testPath);

			expect(result[0].format).toBe(mockMockedCssVariablesFormat);
		});

		it('should use the correct filter from hooks', () => {
			const testPath = '/test/path';
			const result = cssOutputFileConfig(testPath);

			expect(result[0].filter).toBe(mockMockedExcludedTokensFilterName);
		});

		it('should append .css extension to destination', () => {
			const testPath = '/test/output/tokens-theme';
			const result = cssOutputFileConfig(testPath);

			expect(result[0].destination).toBe(`${testPath}.css`);
			expect(result[0].destination?.endsWith('.css')).toBe(true);
		});
	});

	describe('edge cases and error scenarios', () => {
		it('should handle null input by creating "null.css" destination', () => {
			const scenario = createNullInputScenario();

			const result = cssOutputFileConfig(scenario.input);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0].destination).toBe('null.css');
			expect(result[0].format).toBe(mockMockedCssVariablesFormat);
			expect(result[0].filter).toBe(mockExcludedTokensFilterName);
		});

		it('should handle undefined input by creating "undefined.css" destination', () => {
			const scenario = createUndefinedInputScenario();

			const result = cssOutputFileConfig(scenario.input);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0].destination).toBe('undefined.css');
			expect(result[0].format).toBe(mockMockedCssVariablesFormat);
			expect(result[0].filter).toBe(mockExcludedTokensFilterName);
		});

		it('should handle very long file paths', () => {
			const longPath = '/very/'.repeat(100) + 'long/path/to/tokens';
			const result = cssOutputFileConfig(longPath);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0].destination).toBe(`${longPath}.css`);
		});

		it('should handle paths with unicode characters', () => {
			const unicodePath = '/path/with/ünïcödé/characters/тökeиs';
			const result = cssOutputFileConfig(unicodePath);

			expect(result).toBeInstanceOf(Array);
			expect(result).toHaveLength(1);
			expect(result[0].destination).toBe(`${unicodePath}.css`);
		});
	});

	describe('dependency integration', () => {
		it('should use the excludedTokensFilterName from hooks module', () => {
			const testPath = '/test/path';
			const result = cssOutputFileConfig(testPath);

			// The mock value should be used
			expect(result[0].filter).toBe(mockMockedExcludedTokensFilterName);
		});

		it('should use cssVariables format from style-dictionary enums', () => {
			const testPath = '/test/path';
			const result = cssOutputFileConfig(testPath);

			// Should use the actual format from style-dictionary
			expect(result[0].format).toBe(mockMockedCssVariablesFormat);
		});
	});

	describe('function behavior consistency', () => {
		it('should return consistent results for same input', () => {
			const testPath = '/consistent/test/path';

			const result1 = cssOutputFileConfig(testPath);
			const result2 = cssOutputFileConfig(testPath);

			expect(result1).toEqual(result2);
			expect(result1).not.toBe(result2); // Should be different objects
		});

		it('should return different destinations for different inputs', () => {
			const path1 = '/path/one';
			const path2 = '/path/two';

			const result1 = cssOutputFileConfig(path1);
			const result2 = cssOutputFileConfig(path2);

			expect(result1[0].destination).not.toBe(result2[0].destination);
			expect(result1[0].destination).toBe(`${path1}.css`);
			expect(result2[0].destination).toBe(`${path2}.css`);
		});

		it('should always return exactly one file configuration', () => {
			const testPaths = ['/test/path1', '', '/very/long/path/with/many/segments', '/path with spaces', '/path/with-special_chars@#'];

			testPaths.forEach(path => {
				const result = cssOutputFileConfig(path);
				expect(result).toHaveLength(1);
			});
		});
	});

	describe('integration with commented code', () => {
		it('should not include commented out reference file configuration', () => {
			const testPath = '/test/path';
			const result = cssOutputFileConfig(testPath);

			// Should only have one file, not two (the commented out ref file should not be included)
			expect(result).toHaveLength(1);
			expect(result[0].destination).toBe(`${testPath}.css`);
			expect(result[0].destination).not.toContain('-ref.css');
		});
	});
});
